...
function CreateFrontStore(props) {
    ...
    return (
        <AppModal title={"Create Front Store"} isOpen={createStore} close={() => close()}>
            <form className="w-100" onSubmit={handleSubmit}>

                <div className="form-group mh-40 ">
                    <label className="font-12 font-medium">Store Name</label>
                    <input
                    ...
                    <div className="d-flex justify-content-end mt-1">
                        {values && values.storeName && (
                            validating ? (
                                <span className="font-10 text-muted">checking...</span>
                            ) : (
                ...
                <div className="form-group mh-40 ">
                    <label className="font-12 font-medium">Store Url</label>
                ...
                <div className="form-group mh-40 ">
                    ...
                    <select
                        ...
                        required
                    >
                        <option selected disabled>{!isEmpty(currencies) && currencies.payload ? "Select currency" : "Loading..."}</option>
                        {
                            !isEmpty(currencies) &&
                            currencies.payload &&
                ...
                <div className="form-group mh-40 ">
                    <div className="d-flex align-items-center mb-2">
                        <span className="font-12 font-medium mr-2">Split Settlement</span>
                        <input type="checkbox" checked={splitSettlement} onClick={()=>setSplitSettlement(!splitSettlement)}/>
                    </div>
                    {splitSettlement && <input
                        placeholder="Enter sub-account ID to split settlement with"
                        className="form-control mh-40"
                        type="text"
                        name="subAccountId"
                        onChange={(e) => handleValue(e)}
