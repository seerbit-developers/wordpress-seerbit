...
function CreateFronstore(props) {
  const {
    validateCustomUrl,
    validatecustomurl,
    currencies,
  ...
  useEffect(() => {
    if (isEdit && !isEmpty(selectedData && selectedData.additionalData)) {
      let data = [];
      const splitData = selectedData.additionalData.split("||");
      for (let value of splitData) {
        data.push(value.substring(0, value.length - 5))
      }
  ...
  const handleSubmit = (e) => {
    ...
    if (!isEdit) {
      ...
      const payload = {
            ...
            expiryDuration: newPaymentLink.expiryDuration ? newPaymentLink.expiryDuration : 0,
          ...
          createPaymentLink(payload)
              .then(res => {
                ...
                if (res.responseCode === '00'){
                  alertSuccess(!isEmpty(res.message) ? res.message : 'Payment link created');
                  formReset()
                  reload()
                }else{
  ...
  return (
    <AppModal title={isEdit ? "Update Payment Link" : "Create Payment Link"} isOpen={isOpen} close={() => close()} >
      <form className="w-100 pb-4" onSubmit={handleSubmit} id='seerbit-form'>
        ...
        {!isEdit && <div className="form-group mh-40 ">
        ...
        <div className="my-2">
          ...
          <div className="font-12">
            {
              t('Single use links are automatically deactivated once payment has been recieved while multiple use links can be used multiple times.')
            }
          </div>
        ...
        {showAdditionalFields && <div>
          <Modal.Title className="font-18 text-dark pb-1">
            <div className="pt-2 text-bold">
              <strong>{t('additionalFields (optional)')}</strong>
            </div>
          </Modal.Title>

          <div className="form-group mh-40 ">
            <label className="font-12 font-medium">{t('Custom PaymentLink')}</label>
            <div className="input-group">
          ...
          <div className="form-group mh-40 ">
            <label className="font-12 font-medium">{t('splitPayment')}</label>
            <input
          ...
          <div className="form-group mh-40 ">
            <label className="font-12 font-medium">{t('redirectUrl')}</label>
            <input
              className="form-control mh-40 "
          ...
          <div className="form-group mh-40 mb-4">
            <label className="font-12 font-medium">{t('customField')}</label>
            <div className="input-group">
              <input
                className="form-control mh-40 "
