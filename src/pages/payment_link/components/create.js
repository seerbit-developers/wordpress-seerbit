...
function CreatePaymentLink(props) {
  ...
  const [showMobileNumber, setShowMobileNumber] = useState(
    isEdit ? selectedData.requiredFields.mobileNumber : false
  );
  const [active, setActive] = useState(
    isEdit ? (selectedData.status === "ACTIVE" ? true : false) : true
  );
  const [type, setType] = useState(isEdit ? selectedData.oneTime : true);
  const [paymentFrequency, setFrequency] = useState(
    isEdit ? selectedData.paymentFrequency : "ONE_TIME"
  );
  const [showAmount, setShowAmount] = useState(
    isEdit ? selectedData.requiredFields.amount : false
  );
  const [showDate, setShowDate] = useState(
    isEdit ? selectedData.linkExpirable : false
  );
  ...
  //         setProcessing(false);
  //         alertError(error_details.message ? error_details.message : "Unable to complete the action at the moment. Please try again soon.");
  //     }
  ...
  useEffect(() => {
    if (newPaymentLink && newPaymentLink.customizationName) {
      ...
      const timer = setTimeout(
        () => validateCustomUrl({ data: newPaymentLink.customizationName }),
        time
      );
  ...
  const handleSubmit = (e) => {
    ...
    setProcessing(true);
    if (isEmpty(newPaymentLink.currency)) {
      alertError("Please select a currency");
      setProcessing(false);
      return;
    }
    const payload = {
      ...
      expiryDuration: newPaymentLink.expiryDuration
        ? newPaymentLink.expiryDuration
        : 0,
      customTime: newPaymentLink.customTime ? newPaymentLink.customTime : "",
    ...
    createPaymentLink(payload)
      .then((res) => {
        ...
        if (res.responseCode === "00") {
          alertSuccess("Payment link created");
          formReset();
          reload();
          close();
        } else {
  ...
  const handleValue = (e) => {
    if (e.target.name === "customizationName") {
      let removeSpecialCharacters = e.target.value
        .replace(/[^a-zA-Z ]/g, "")
        .trim();
    ...
    if (newField !== "" && newField.length > 2) {
      ...
      if (isAvailable) {
        ...
        const uniqueFields = updatedFields.filter(
          (field, index) => updatedFields.indexOf(field) === index
        );
  ...
  const removeField = (index) => {
    setAdditionalField([
      ...additionalFields.filter(
        (field) => additionalFields.indexOf(field) !== index
      ),
    ]);
  };
  ...
  const convertDateToMinute = (dateFuture) => {
    ...
    const startDate = moment(currentDate.getTime()).format(
      "yyyy-MM-DD HH:mm:ss"
    );
    setDate(dateFuture);
    ...
    setPaymentLink({
      ...
      customTime: startDate,
    });
  };

  return (
    <AppModal
      title={isEdit ? "Update Payment Link" : "Create Payment Link"}
      isOpen={isOpen}
      close={() => close()}
    >
      <form className="w-100 pb-4" onSubmit={handleSubmit} id="seerbit-form">
            ...
            <div className="input-group">
              <div className="input-group-prepend">
                <span className="input-group-text" id="basic-addon1">
                  {isEdit && newPaymentLink.currency === undefined
                    ? selectedData.currency !== undefined
                      ? selectedData.currency
                      : business_details.default_currency
                    : newPaymentLink.currency !== undefined
                    ? newPaymentLink.currency
                    : business_details.default_currency}
                </span>
              </div>
              <input
                ...
                required
                value={
                  isEdit && newPaymentLink.amount === undefined
                    ? selectedData.amount
        ...
        <div className=" form-inline">
          ...
          <label className="form-label font-12">
            {t("I want to collect a fix amount on this page")}.
          </label>
        </div>
        <div className="mb-2">
          <div className="form-inline">
            ...
            <label className="form-label font-12">
              {t("I want to set expiration period")}.
            </label>
          </div>
          ...
          <div className="form-group mb-0 form-inline">
            ...
            <label className="form-label font-12">
              {t("I want to collect phone number")}.
            </label>
          </div>
          ...
          <div className="form-inline">
            ...
            <label className="form-label font-12">
              {t("I want to set the status to be active")}.
            </label>
          </div>
        </div>
        ...
        <div className="mb-3">
          <div className="form-group mh-40 ">
            <label className="font-12">{t("Accept Specific Currency")}</label>
            <select
              className="form-control mh-40"
              name="currency"
              onChange={(e) => handleValue(e)}
            ...
            >
              <option selected value={null} disabled>
                {!isEmpty(currencies) && currencies.payload
                  ? t("Select Currency")
                  : t("Loading") + "..."}
              </option>
              {!isEmpty(currencies) &&
          ...
          {showDate && (
            <div className="sbt-expiration">
              <label className="font-12">{t("Select expiration period")}</label>
              <Calendar
                showTime
                hourFormat="12"
                placeholder={t("Select Expiration Period")}
                minDate={new Date()}
                value={date}
        ...
        <div className="my-2">
          ...
          <div className="font-12">
            {t(
              "Single use links are automatically deactivated once payment has been received while multiple use links can be used multiple times"
            )}
            .
          </div>
          <div className="font-12 my-2">
            <div className="form-group mb-0 form-inline">
              ...
              <label className="form-label mx-2">{t("Single Use")}</label>
            </div>
            <div className="form-group mb-0 form-inline">
              <div className="form-group">
                <input
              ...
              <label className="form-label mx-2">{t("Multiple Use")}</label>
            </div>
          </div>
        </div>

        <div className="my-2">
          <div className="font-12 my-2 font-medium">
            {t("Payment Frequency")}
          </div>
          <div className="font-12">
            {t(
          ...
          <div className="font-12 my-2">
            <div className="form-group mb-0 form-inline">
              ...
              <label className="form-label mx-2">{t("One time")}</label>
            </div>
            <div className="form-group mb-0 form-inline">
              <div className="form-group">
                <input
              ...
              <label className="form-label mx-2">{t("Recurring")}</label>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="form-group mh-40 ">
              ...
              <div className="input-group">
                ...
                <input
                  ...
                  type="text"
                  value={
                    isEdit && newPaymentLink.customizationName === undefined
                      ? selectedData.customizationName
                      : newPaymentLink.customizationName
                  }
                  onChange={(e) => handleValue(e)}
                />
              </div>
                ...
                {newPaymentLink &&
                  ...
                  (validating ? (
                    ...
                    <span className="font-10">
                      {validatecustomurl && !validatecustomurl.payload ? (
            ...
            <div className="form-group mh-40 ">
              ...
              <input
                ...
                onChange={(e) => handleValue(e)}
                value={
                  isEdit && newPaymentLink.successMessage === undefined
                    ? selectedData.successMessage
                    : newPaymentLink.successMessage
                }
              />
            </div>
            ...
            <div className="form-group mh-40 ">
              ...
              <input
                ...
                onChange={(e) => handleValue(e)}
                value={
                  isEdit && newPaymentLink.vendorId === undefined
                    ? selectedData.vendorId
                    : newPaymentLink.vendorId
                }
              />
            </div>
            ...
            <div className="form-group mh-40 ">
              ...
              <input
                ...
                onChange={(e) => handleValue(e)}
                value={
                  isEdit && newPaymentLink.redirectUrl === undefined
                    ? selectedData.redirectUrl
            ...
            <div className="px-3 row">
              {additionalFields &&
                additionalFields.map((field, id) => (
                  <div
                    key={id}
                    className="tag my-2 mr-2 font-12 text-capitailize"
                  >
                    <span>{field}</span>
                    <span className="badge" onClick={() => removeField(id)}>
                      X
                    </span>
                  </div>
                ))}
