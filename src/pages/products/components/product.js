...
import {updateProduct, updateProductImage,createProduct} from "services/productService";
...
import {dispatchProductDetails} from "actions/frontStoreActions";
...
function Product({
  ...
  const fillForm = ()=>{
    if (product){
      ...
      setIsDepletable(product.isDepletable)
      setAmount(product.amount)
      setQuantity(product.quantity)
      setStatus(product.status)
      setCategoryId(product.category ? product.category.id : '')
      if (Array.isArray(product.productImages)){
        const imgs = product.productImages.map( img=> img.productImageUrl )
        setFiles(imgs)
      }
    }
  }
        ...
        updateProduct(
          ...
          setProductProcessing(false);
            if (res.responseCode === '00'){
              ...
              dispatch(dispatchProductDetails(res.payload))
              if (files && files.length && updateImage)
              {
                const upload = initUpload(product.id);
  ...
  return (
    ...
    <AppModal title={`${type} ${t('Product')}`} isOpen={isOpen} close={() => close()}>
        <form
        ...
        >
          ...
          {/*      <strong>400</strong> and greater than <strong>750</strong>,*/}
          ...
          <div className="row">
            ...
            <div className="col-12">
              ...
              >
                ...
                {!productProcessing && `${type} ${t('Product')}`}
              </Button>
            </div>
          </div>
        </form>
